...
void main() {
  group('EnumHelpers Tests', () {
    ...
    test('tryParseEnum handles valid input (case-insensitive)', () {
      expect(
          EnumHelpers.tryParseEnum(testValues, 'high'), equals(Priority.high));
      expect(EnumHelpers.tryParseEnum(testValues, 'MEDIUM'),
          equals(Priority.medium));
      expect(EnumHelpers.tryParseEnum(testValues, 'lOw'), equals(Priority.low));
      expect(EnumHelpers.tryParseEnum(testValues, ' high '),
          equals(Priority.high)); // Handles whitespace
    });
    ...
    test('getEnumValuesAsString returns correct comma-separated string', () {
      expect(EnumHelpers.getEnumValuesAsString(testValues),
          equals(testNamesString));
    });
  ...
  group('Priority Tests', () {
    test('description returns correct strings', () {
      expect(Priority.high.description,
          'Critical tasks needing immediate attention');
      expect(
          Priority.medium.description, 'Important tasks to be addressed soon');
  ...
  group('TaskStatus Tests', () {
    test('description returns correct strings', () {
      ...
      expect(
          TaskStatus.inProgress.description, 'Task currently being worked on');
    ...
    test('tryParse works correctly', () {
      ...
      expect(TaskStatusExtension.tryParse('INPROGRESS'),
          equals(TaskStatus.inProgress));
      expect(TaskStatusExtension.tryParse(' blocked '),
    ...
    test('allowedValuesString returns correct string', () {
      expect(TaskStatusExtension.allowedValuesString,
          'toDo, inProgress, blocked, done');
    });

    test('names returns correct list', () {
      expect(TaskStatusExtension.names,
          orderedEquals(['toDo', 'inProgress', 'blocked', 'done']));
    });
  });
  ...
  group('GoalStatus Tests', () {
    test('description returns correct strings', () {
      ...
      expect(
          GoalStatus.inProgress.description, 'Goal currently being worked on');
      expect(GoalStatus.achieved.description, 'Goal successfully completed');
    });

    test('tryParse works correctly', () {
      expect(GoalStatusExtension.tryParse('notStarted'),
          equals(GoalStatus.notStarted));
      expect(GoalStatusExtension.tryParse('PLANNING'),
          equals(GoalStatus.planning));
      expect(GoalStatusExtension.tryParse(' inProgress '),
          equals(GoalStatus.inProgress));
      expect(GoalStatusExtension.tryParse('achieved'),
          equals(GoalStatus.achieved));
      expect(GoalStatusExtension.tryParse('invalid'), isNull);
      expect(GoalStatusExtension.tryParse(null), isNull);
    });

    test('allowedValuesString returns correct string', () {
      expect(GoalStatusExtension.allowedValuesString,
          'notStarted, planning, inProgress, achieved');
    });

    test('names returns correct list', () {
      expect(GoalStatusExtension.names,
          orderedEquals(['notStarted', 'planning', 'inProgress', 'achieved']));
    });
  });
  ...
  group('PlanType Tests', () {
    test('description returns correct strings', () {
      ...
      expect(PlanType.tactical.description,
          'Mid-level plan to achieve strategic goals');
      expect(PlanType.operational.description,
          'Low-level, day-to-day action plan');
    });

    test('tryParse works correctly', () {
      expect(
          PlanTypeExtension.tryParse('strategic'), equals(PlanType.strategic));
      expect(PlanTypeExtension.tryParse('TACTICAL'), equals(PlanType.tactical));
      expect(PlanTypeExtension.tryParse(' operational '),
          equals(PlanType.operational));
      expect(PlanTypeExtension.tryParse('invalid'), isNull);
      expect(PlanTypeExtension.tryParse(null), isNull);
    });

    test('allowedValuesString returns correct string', () {
      expect(PlanTypeExtension.allowedValuesString,
          'strategic, tactical, operational');
    });

    test('names returns correct list', () {
      expect(PlanTypeExtension.names,
          orderedEquals(['strategic', 'tactical', 'operational']));
    });
  });
  ...
  group('Likelihood Tests', () {
    ...
    test('names returns correct list', () {
      expect(
          LikelihoodExtension.names, orderedEquals(['high', 'medium', 'low']));
    });
  });
  ...
  group('ObstacleCategory Tests', () {
    test('description returns correct strings', () {
      expect(ObstacleCategory.technical.description,
          'Issues related to technology');
      expect(
          ObstacleCategory.resource.description, 'Lack of necessary resources');
      expect(ObstacleCategory.market.description, 'Challenges from the market');
      expect(ObstacleCategory.behavioral.description,
          'Issues related to team or individual behavior');
      expect(ObstacleCategory.communication.description,
          'Issues related to information flow');
      expect(ObstacleCategory.financial.description,
          'Issues related to budget or funding');
    });

    test('tryParse works correctly', () {
      expect(ObstacleCategoryExtension.tryParse('technical'),
          equals(ObstacleCategory.technical));
      expect(ObstacleCategoryExtension.tryParse('RESOURCE'),
          equals(ObstacleCategory.resource));
      expect(ObstacleCategoryExtension.tryParse(' market '),
          equals(ObstacleCategory.market));
      expect(ObstacleCategoryExtension.tryParse('behavioral'),
          equals(ObstacleCategory.behavioral));
      expect(ObstacleCategoryExtension.tryParse('communication'),
          equals(ObstacleCategory.communication));
      expect(ObstacleCategoryExtension.tryParse('FINANCIAL'),
          equals(ObstacleCategory.financial));
      expect(ObstacleCategoryExtension.tryParse('invalid'), isNull);
      expect(ObstacleCategoryExtension.tryParse(null), isNull);
    });

    test('allowedValuesString returns correct string', () {
      expect(ObstacleCategoryExtension.allowedValuesString,
          'technical, resource, market, behavioral, communication, financial');
    });

    test('names returns correct list', () {
      expect(
          ObstacleCategoryExtension.names,
          orderedEquals([
            'technical',
            'resource',
            'market',
