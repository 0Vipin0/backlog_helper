...
// Helpers...
String getTempDir() => Directory.systemTemp.createTempSync('excel_service_test_').path;
...
void main() {
  ...
  group('Initialization and File Handling', () {
    test('creates new file with sheets and headers if file does not exist on first load', () async {
      final service = ExcelService(filePath: testFilePath); // Instance for this test
      ...
      final headerValues = headerRowData.map((cellData) => cellData?.value?.toString() ?? '').toList();
    ...
    test('loads existing file correctly (and uses cache)', () async {
      ...
      final initialTask = BacklogTask(id: 't-init', taskTitle: 'Initial', priority: Priority.low);
      final firstId = await service1.addItem(initialTask); // Saves & clears cache

      // Use a NEW service instance for initial read from file
      final service2 = ExcelService(filePath: testFilePath);
      final items1 = await service2.getAllItems<BacklogTask>(BacklogTask.fromRow);
      ...
      sheet.appendRow([
        ...
        TextCellValue(Priority.medium.name) // Priority (required - provide valid enum name)
        // Add nulls or valid CellValues for other columns if needed by fromRow
      ]);
      final encodedBytes = excelDirect.encode();
      if(encodedBytes != null) await File(testFilePath).writeAsBytes(encodedBytes); else fail('Encoding failed');

      // Read again using service2 - SHOULD hit cache (no clearCache called on service2)
      final items2 = await service2.getAllItems<BacklogTask>(BacklogTask.fromRow);
      expect(items2.length, equals(items1.length), reason: "Should read from cache");

      // Read after cache clear on service2
      service2.clearCache();
      final items3 = await service2.getAllItems<BacklogTask>(BacklogTask.fromRow);
      expect(items3.length, greaterThan(items1.length), reason: "Should read modified file");
  ...
  group('addItem<T>', () {
    test('adds a new BacklogTask and saves', () async {
      final service = ExcelService(filePath: testFilePath); // Instance for this test
      final task = BacklogTask(id: 'will-be-replaced', taskTitle: 'Test Add Task', priority: Priority.high);
      expect(await getRowCount(testFilePath, ExcelConstants.sheetTasks), 0);

      final generatedId = await service.addItem(task); // Saves & clears cache on 'service'
      ...
      final items = await readerService.getAllItems<BacklogTask>(BacklogTask.fromRow);
    ...
    test('adds a new FutureGoal and saves', () async {
      final service = ExcelService(filePath: testFilePath); // Instance for this test
      final goal = FutureGoal(id: 'g-replace', goalDescription: 'Test Add Goal', targetCompletionDate: '2025-01-01', priority: Priority.medium);
      ...
      final readerService = ExcelService(filePath: testFilePath);
      final items = await readerService.getAllItems<FutureGoal>(FutureGoal.fromRow);
      expect(items.length, 1);
      expect(items.first.id, equals(generatedId));
    });
  });
  ...
  group('getAllItems<T>', () {
    ...
    test('retrieves a single added BacklogTask correctly', () async {
      final service = ExcelService(filePath: testFilePath); // Service for writing
      final task1 = BacklogTask(id:'', taskTitle: 'Single Task', priority: Priority.high);
    ...
    test('retrieves multiple added BacklogTasks correctly', () async {
      final service = ExcelService(filePath: testFilePath); // Service for writing
      final task1 = BacklogTask(id:'', taskTitle: 'Multi Task 1', priority: Priority.high);
      final task2 = BacklogTask(id:'', taskTitle: 'Multi Task 2', priority: Priority.low, description: "Desc 2");
      ...
      expect(items.firstWhere((t) => t.taskTitle == 'Multi Task 2').description, "Desc 2");
    });

    test('retrieves items from correct sheet when multiple types exist', () async {
      final service = ExcelService(filePath: testFilePath); // Service for writing
      final task1 = BacklogTask(id:'', taskTitle: 'Task For Mix', priority: Priority.high);
      final goal1 = FutureGoal(id:'', goalDescription: 'Goal For Mix', targetCompletionDate: '2024', priority: Priority.medium);
      await service.addItem(task1);
      ...
      // Read Tasks
      final taskItems = await readerService.getAllItems<BacklogTask>(BacklogTask.fromRow);
      expect(taskItems.length, 1, reason: "Should find 1 task");
      expect(taskItems.first.taskTitle, equals('Task For Mix'));

      // Read Goals
      final goalItems = await readerService.getAllItems<FutureGoal>(FutureGoal.fromRow);
    ...
    test('skips row if ID is missing/empty', () async {
      ...
      await service.addItem(BacklogTask(id:'', taskTitle: 'Valid Task Before Empty', priority: Priority.medium));
      ...
      final excel = await service.loadExcel(); // Use internal method carefully for test setup
      final sheet = excel[ExcelConstants.sheetTasks];
      sheet.appendRow([TextCellValue(''), TextCellValue('Task With Empty ID'), TextCellValue('high')]);
      final encodedBytes = excel.encode(); if(encodedBytes == null) fail("Encoding failed");
    ...
    test('handles parsing errors gracefully (skips row)', () async {
      final service = ExcelService(filePath: testFilePath);
      // Add a valid item first
      final validTask = BacklogTask(id:'', taskTitle: 'Valid Task Before Bad', priority: Priority.medium);
      ...
      sheet.appendRow([TextCellValue(taskId), TextCellValue('Task Invalid Prio'), TextCellValue('INVALID_PRIORITY_VALUE')]);
      final encodedBytes = excel.encode(); if(encodedBytes == null) fail("Encoding failed");
      await File(testFilePath).writeAsBytes(encodedBytes);
  ...
  group('getItemById<T>', () {
    ...
    test('retrieves correct BacklogTask by ID', () async {
      final service = ExcelService(filePath: testFilePath);
      final t1 = BacklogTask(id:'', taskTitle: 'Task Find Me', priority: Priority.high);
      final task1Id = await service.addItem(t1); // Add item

      // Use a new reader instance
      final readerService = ExcelService(filePath: testFilePath);
      final item = await readerService.getItemById<BacklogTask>(task1Id, BacklogTask.fromRow);
    ...
    test('retrieves correct FutureGoal by ID', () async {
      final service = ExcelService(filePath: testFilePath);
      final g1 = FutureGoal(id:'', goalDescription: 'Goal Find Me', targetCompletionDate: '2024', priority: Priority.medium);
      final goal1Id = await service.addItem(g1); // Add item

      // Use a new reader instance
      final readerService = ExcelService(filePath: testFilePath);
      final item = await readerService.getItemById<FutureGoal>(goal1Id, FutureGoal.fromRow);
      expect(item, isNotNull);
    ...
    test('returns null if ID is not found', () async {
      ...
      await service.addItem(BacklogTask(id: '', taskTitle: 'Other', priority: Priority.low));

      // Use a new reader instance
      final readerService = ExcelService(filePath: testFilePath);
      final item = await readerService.getItemById<BacklogTask>('non-existent-id', BacklogTask.fromRow);
      expect(item, isNull);
    });

    test('returns null if sheet does not exist for the type initially', () async {
      final readerService = ExcelService(filePath: testFilePath);
      // Ensure file exists by adding something else
      await readerService.addItem(BacklogTask(id: '', taskTitle: 'Filler', priority: Priority.low));
      readerService.clearCache(); // Clear cache after write

      // Now try to get item from a sheet that wasn't explicitly added
      final item = await readerService.getItemById<PlanningItem>('some-id', PlanningItem.fromRow);
      expect(item, isNull);
    ...
    test('handles parsing error for the specific row gracefully', () async {
      final service = ExcelService(filePath: testFilePath);
      final t1 = BacklogTask(id:'', taskTitle: 'Task To Corrupt', priority: Priority.high);
      ...
      final sheet = excel[ExcelConstants.sheetTasks];
      int? rowIndex;
      for (int i = 1; i < sheet.maxRows; i++) { // Start from 1 (skip header)
      ...
      expect(rowIndex, isNotNull, reason: "Row with ID $task1Id not found after addItem/reload");
      ...
      final item = await readerService.getItemById<BacklogTask>(task1Id, BacklogTask.fromRow);
      expect(item, isNull, reason: "getItemById should return null when parsing fails");
    });
  ...
  group('updateItem<T>', () {
    ...
    test('updates existing BacklogTask data successfully', () async {
      final service = ExcelService(filePath: testFilePath);
      // 1. Add initial item
      final t1 = BacklogTask(id:'', taskTitle: 'Task Before Update', priority: Priority.high, description: "Old Desc");
      final task1Id = await service.addItem(t1);

      // 2. Get the item using a new instance to ensure we have the saved state
      final readerService = ExcelService(filePath: testFilePath);
      final taskToUpdate = await readerService.getItemById<BacklogTask>(task1Id, BacklogTask.fromRow);
      expect(taskToUpdate, isNotNull, reason: "Failed to read item back after initial add"); // Check setup worked
      ...
      await Future.delayed(const Duration(milliseconds: 200)); // Adjust if needed
      // --- END DELAY ---

      // 5. Verify by reloading using another new service instance
      final verifierService = ExcelService(filePath: testFilePath);
      final updatedItem = await verifierService.getItemById<BacklogTask>(task1Id, BacklogTask.fromRow);

      // This is the failing assertion
      expect(updatedItem, isNotNull, reason: "getItemById returned null after update");
      ...
      expect(updatedItem.updatedAt.isAtSameMomentAs(taskToUpdate.updatedAt), isTrue);
    ...
    test('returns false if item ID is not found for update', () async {
      ...
      final nonExistentTask = BacklogTask(id: 'fake-id', taskTitle: 'Wont Find', priority: Priority.low);
      final updateService = ExcelService(filePath: testFilePath);
    ...
    test('returns false if sheet does not exist for update', () async {
      ...
      final nonExistentPlan = PlanningItem(id: 'plan-id', planItemDescription: 'No Sheet', typeOfPlan: PlanType.strategic, status: 'Ok');
      final updateService = ExcelService(filePath: testFilePath);
