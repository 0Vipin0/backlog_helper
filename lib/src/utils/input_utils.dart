...
class InputUtils {
  ...
  static String? prompt(String message, {bool isRequired = false, String? currentValue, bool Function(String)? validator, String? validationError}) {
    String? input;
    bool firstPrompt = true;
    String displayCurrent = currentValue != null && currentValue.isNotEmpty ? ' [Current: $currentValue]' : '';
    ...
    do {
      ...
      if (!firstPrompt) {
        ...
        } else if (validator != null && input != null && input.isNotEmpty && !validator(input)) {
      ...
      if (currentValue != null && input != null && input.isEmpty && !isRequired) {
          return currentValue; // Keep the current value
      }

    } while (
        (isRequired && (input == null || input.isEmpty)) ||
        (validator != null && input != null && input.isNotEmpty && !validator(input))
     );
  ...
  static String? promptEnum<T extends Enum>(String message, List<T> enumValues, {bool isRequired = false, String? currentValue}) {
    String? input;
    bool firstPrompt = true;
    T? parsedEnum;
    String displayCurrent = currentValue != null && currentValue.isNotEmpty ? ' [Current: $currentValue]' : '';
    ...
    // Return the input string if valid or null if empty/invalid optional
    return (input != null && input.isNotEmpty && parsedEnum != null) || (!isRequired && (input == null || input.isEmpty))
           ? input
           : null;
  }
  ...
  static bool isValidDate(String? input) {
    if (input == null || input.isEmpty) return true; // Allow empty optional dates
