...
class DisplayUtils {
  ...
  static void printTable<T extends ExcelStorable>(List<T> items, List<String> headers) {
    if (items.isEmpty) {
    ...
    final tableData = items.map((item) {
      ...
      // Add a method to the models e.g., toListForDisplay()
       ...
       print("Warning: Using fallback display logic for item type ${item.runtimeType}. Implement 'toListForDisplay()' in '${item.runtimeType}' for optimal formatting.");
