...
class InteractiveCommand extends Command<void> {
  ...
  final ExcelService _excelService; // Allow passing service
   ...
   InteractiveCommand({ExcelService? excelService})
      : _excelService = excelService ?? ExcelService(); // Use default service if none provided
  ...
  Future<void> run() async {
    ...
    ..addCommand(ListCommand(excelService: _excelService)) // List doesn't need interactive flag, but needs service
    ..addCommand(UpdateCommand(excelService: _excelService, interactive: true));
    ...
    while (true) {
      ...
      if (line == null || ['exit', 'quit'].contains(line.trim().toLowerCase())) {
