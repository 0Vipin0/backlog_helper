...
class AddTaskCommand extends Command<void> {
  ...
  AddTaskCommand({required ExcelService excelService, this.interactive = false})
    ...
    argParser.addOption('description',
        abbr: 'd',
        help: 'Please provide a detailed description of the task.',
        valueHelp: 'DESCRIPTION');
    argParser.addOption('effort',
        abbr: 'e',
        help:
            "What's your best estimate of the effort needed? (e.g., hours, days)",
        valueHelp: 'EFFORT_ESTIMATE');
    argParser.addOption('status',
        abbr: 's',
        help:
            'What is the current status of this task? \nAllowed: [${TaskStatusExtension.allowedValuesString}]',
        valueHelp: 'STATUS',
    ...
    argParser.addOption('due-date',
        help:
            'Is there a specific date this task should be done by? (YYYY-MM-DD)',
        valueHelp: 'YYYY-MM-DD');
    argParser.addOption('tags',
        help:
            'Are there any tags or categories for this task? (e.g., UI, Bug, Feature)',
        valueHelp: 'tag1,tag2');
    argParser.addOption('depends-on',
        help:
            'Are there any tasks this task depends on? (Comma-separated Task Titles)',
        valueHelp: 'title1,title2');
    argParser.addOption('reason',
        help:
            'What is the reason or goal behind this task? (Related Goal/Plan Title)',
        valueHelp: 'GOAL_OR_PLAN_TITLE');
    argParser.addOption('resolution',
        help:
            'What was the resolution for this task? (For completed/blocked tasks)',
        valueHelp: 'RESOLUTION_NOTES');
  }
  ...
  Future<void> run() async {
    ...
    if (interactive) {
      ...
      title = InputUtils.prompt('What is the task?',
          isRequired: true, currentValue: title);
      priorityStr = InputUtils.promptEnum<Priority>(
          'How important is this task? (${PriorityExtension.allowedValuesString})',
          Priority.values,
          isRequired: true,
          currentValue: priorityStr);
      description = InputUtils.prompt(
          'Please provide a detailed description (Optional):',
          currentValue: description);
      effort = InputUtils.prompt(
          "What's your estimate of the effort needed? (Optional):",
          currentValue: effort);
      statusStr = InputUtils.promptEnum<TaskStatus>(
          'What is the current status? (${TaskStatusExtension.allowedValuesString}) (Optional):',
          TaskStatus.values,
          currentValue: statusStr);
      dueDate = InputUtils.prompt('Due date? (YYYY-MM-DD) (Optional):',
          validator: InputUtils.isValidDate,
          validationError: 'Invalid date format.',
          currentValue: dueDate);
      tags = InputUtils.prompt(
          'Tags or categories? (comma-separated) (Optional):',
          currentValue: tags);
      dependencies = InputUtils.prompt(
          'Depends on which tasks? (comma-separated titles) (Optional):',
          currentValue: dependencies);
      reason = InputUtils.prompt('Reason/Goal behind this task? (Optional):',
          currentValue: reason);
      resolution = InputUtils.prompt('Resolution notes? (Optional):',
    ...
    if (priority == null) {
      // Should be caught by argParser allowed, but check anyway
    ...
    try {
      ...
      print(
          '❌ Error adding task to Excel file "${_excelService.filePath}": $e');
