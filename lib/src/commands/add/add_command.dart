...
class AddCommand extends Command<void> {
  ...
  @override
  final String description = 'Add a new item (task, goal, plan, obstacle) to the Excel file.';
  ...
  AddCommand({ExcelService? excelService, this.interactive = false})
      : _excelService = excelService ?? ExcelService() { // Use default if not provided
    // Pass service and interactive flag to subcommands
    addSubcommand(AddTaskCommand(excelService: _excelService, interactive: interactive));
    addSubcommand(AddGoalCommand(excelService: _excelService, interactive: interactive));
    addSubcommand(AddPlanCommand(excelService: _excelService, interactive: interactive));
    addSubcommand(AddObstacleCommand(excelService: _excelService, interactive: interactive));
  }
